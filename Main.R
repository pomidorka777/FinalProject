#authors: Sudarev Denis, Baiburin Yusuf
...
axes + theme_classic() +
  scale_fill_brewer(palette = 11, name = "Legend",
                    labels = c("First Half", "Second Half")) +
  geom_bar(stat = "identity", position = "stack") +
  geom_hline(yintercept = mean(clean_data$total),
             color = "black", size = 0.8, show.legend = TRUE) +
  geom_label(aes(label = goals),
             position = "stack", vjust = 1.2, label.size = 0.5) +
  labs(title = "Number of goals scored in group stage in FIFA 2018 World Cup (by periods)",
        x = "Group", y = "Number of goals")